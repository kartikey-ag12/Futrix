...
import { 
  ChevronDown, Filter, ArrowUpDown, Calendar, AlertTriangle, CheckCircle2, XCircle, LayoutTemplate
} from "lucide-react";
...
export default function DueInvoicesReviewPage() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#f5f5f5] dark:bg-[#0a0a0a] min-h-screen relative overflow-hidden">
      
      {/* Progress Bar */}
      <div className="w-full h-1 bg-white/10 shrink-0">
        <div className="h-full bg-emerald-500 transition-all duration-500 ease-out" style={{ width: '40%' }} />
      </div>

      <div className="flex-1 flex flex-col p-8 overflow-y-auto">
        
        {/* Header */}
        <div className="flex items-start justify-between max-w-7xl mx-auto w-full mb-8">
          <div>
            <h1 className="text-2xl font-semibold text-foreground mb-1">Due invoices and bills are included in your cash flow...</h1>
            <p className="text-foreground/60 text-sm">Overdue invoices and bills are always counted in the forecasted bank position for today.</p>
          </div>
          <button 
            onClick={() => router.push("/forecasting")}
            className="px-8 py-2.5 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white font-medium transition-all shadow-sm"
          >
            Next
          </button>
        </div>
        ...
        <div className="max-w-7xl mx-auto w-full flex flex-col flex-1">
          ...
          <div className="flex items-center gap-1 border-b border-[#e5e5e5] dark:border-white/10 mb-6">
            ...
            <button 
              ...
              className={clsx(
                "px-5 py-2.5 text-sm font-medium border-b-2 transition-colors",
                activeTab === "bills" ? "border-emerald-500 text-foreground" : "border-transparent text-foreground/50 hover:text-foreground"
              )}
            >
              Bills
            </button>
            <button 
              ...
              className={clsx(
                "px-5 py-2.5 text-sm font-medium border-b-2 transition-colors",
                activeTab === "recurring" ? "border-emerald-500 text-foreground" : "border-transparent text-foreground/50 hover:text-foreground"
              )}
            >
              Recurring
            </button>
          ...
          {/* Table */}
          <div className="bg-white dark:bg-[#111] border border-[#e5e5e5] dark:border-white/10 rounded-xl shadow-sm flex flex-col flex-1 overflow-hidden">
